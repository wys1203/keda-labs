...
kubectl apply -f "${KDW_DIR}/demo/demo-deprecated/namespace.yaml"
kubectl apply -f "${KDW_DIR}/demo/demo-deprecated/deployment.yaml"
set +e
APPLY_OUT="$(kubectl apply -f "${KDW_DIR}/demo/demo-deprecated/scaledobject.yaml" 2>&1)"
APPLY_RC=$?
set -e
echo "${APPLY_OUT}"
...
log "restoring CM"
kubectl apply -f "${KDW_DIR}/manifests/deploy/configmap.yaml"
