...
{
  extract_images keda kedacore/keda --version "${KEDA_CHART_VERSION}" -f "${LAB_DIR}/keda/values.yaml"
  extract_images cert-manager jetstack/cert-manager --version "${CERT_MANAGER_VERSION}" --set crds.enabled=true
  extract_images prometheus prometheus-community/prometheus -f "${LAB_DIR}/prometheus/values.yaml"
  extract_images grafana grafana/grafana -f "${LAB_DIR}/grafana/values.yaml"
